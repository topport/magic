...
func (r *FSRepo) openDatastore() error {
	fmt.Println(filepath.Join(r.Path(), defaultDatastoreFolderName))
	d, err := badgerDatastore(filepath.Join(r.Path(), defaultDatastoreFolderName))
	fmt.Println(err,"fuck3")
	if err != nil {
		return err
	}
...
func badgerDatastore(path string) (datastore.Batching, error) {
	//opt :=
	opt:=&badgerds.Options{

	}
	opt.Options=badger.DefaultOptions(path)
