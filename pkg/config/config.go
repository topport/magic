...
import (
	...
	"github.com/topport/magic/pkg/util/bytesize"
)
...
type Server struct {
	...
	Config         string `def:"~/.magic.yml" desc:"location of config file" mapstructure:"config"`
	LogLevel       string `def:"info" desc:"log level: debug|info|warn|error" mapstructure:"log-level"`
	BadgerLogLevel string `def:"error" desc:"log level: debug|info|warn|error" mapstructure:"badger-log-level"`

	StoragePath string `def:"/doc/magicrepo" desc:"directory where pyroscope stores profiling data" mapstructure:"storage-path"`
	APIBindAddr string `def:"4321" desc:"port for the HTTP(S) server used for data ingestion and web UI" mapstructure:"api-bind-addr"`
	RedisBindAddr string `def:":4322" desc:"port for the HTTP(S) server used for data ingestion and web UI" mapstructure:"redis-bind-addr"`
